...
export default function VisionDetailPage() {
  ...
  const fetchVisionDetails = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/coordinador/visiones/${visionId}?t=${Date.now()}`, {
        cache: 'no-store'
      });
  ...
  const fetchCredits = async () => {
    try {
      const res = await fetch('/api/coordinador/dashboard');
      const data = await res.json();
      if (data.success) {
  ...
  const fetchMentores = async () => {
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/mentores`);
      const data = await res.json();
  ...
  const handleRemoverMentor = async (mentorId: number) => {
    ...
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/mentores?mentorId=${mentorId}`, {
        method: 'DELETE',
      });
  ...
  const handleBulkAssignLicenses = async () => {
    ...
    for (const userId of Array.from(selectedUsers)) {
      try {
        const res = await fetch(`/api/coordinador/visiones/${visionId}/assign-license`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleOpenAssignMentorModal = async (userId: number, userType: string, userName: string, hasLicense: boolean, hasMentor: boolean = false) => {
    ...
    if (hasMentor) {
      try {
        const checkRes = await fetch(`/api/coordinador/visiones/${visionId}/check-mentor-impact?userId=${userId}&userType=${userType}`);
    ...
    router.push(`/dashboard/coordinador/visiones/${visionId}/asignar-mentor/${userId}`);
  };
  ...
  const confirmRemoveMentor = async (userId: number, userType: string) => {
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/assign-mentor?userId=${userId}&userType=${userType}`, {
        method: 'DELETE',
      });
  ...
  const handleAsignacionAleatoria = async () => {
    try {
      ...
      const res = await fetch(`/api/coordinador/visiones/${visionId}/assign-mentors-random`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const handleExtendDate = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/coordinador/visiones/${visionId}/extend-date`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAssignGameChanger = async (participanteId: number, gameChangerId: number | null) => {
    try {
      ...
      const res = await fetch(`/api/coordinador/visiones/${visionId}/assign-gamechanger`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRandomAssignment = async () => {
    try {
      setRandomAssigning(true);
      const res = await fetch(`/api/coordinador/visiones/${visionId}/random-assign`, {
        method: 'POST',
      });
  ...
  const handleAddEmails = async () => {
    ...
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/add-emails`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddTeam = async () => {
    ...
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/add-gamechangers`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRemoveParticipante = async (participanteRelationId: number) => {
    ...
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}/remove-participante`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleUpdateAreasConfig = async () => {
    try {
      const res = await fetch(`/api/coordinador/visiones/${visionId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
                          ...
                          {(() => {
                            try {
                              const dateStr = typeof vision.startDate === 'string' 
                                ? vision.startDate 
                                : (vision.startDate as any)?.toISOString?.() || '';
                          ...
                          {(() => {
                            try {
                              const dateStr = typeof vision.endDate === 'string' 
                                ? vision.endDate 
                                : (vision.endDate as any)?.toISOString?.() || '';
                              if (!dateStr) return 'No definida';
                              const date = new Date(dateStr.includes('T') ? dateStr : dateStr + 'T00:00:00');
                              return date.toLocaleDateString('es-MX', {
                                year: 'numeric',
