...
export default function AsignarMentorPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch usuario
      const userRes = await fetch(`/api/coordinador/usuarios/${userId}`);
      ...
      const mentoresRes = await fetch(`/api/coordinador/visiones/${visionId}/mentores`);
      ...
      const visionRes = await fetch(`/api/coordinador/visiones/${visionId}`);
  ...
  const handleAsignarMentor = async (mentorId: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/coordinador/visiones/${visionId}/assign-mentor`, {
        method: 'POST',
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          router.push(`/dashboard/coordinador/visiones/${visionId}`);
        }, 1500);
      } else {
        showToast({
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link 
            href={`/dashboard/coordinador/visiones/${visionId}`}
