...
interface CartaData {
  ...
  finanzasIdentidad: string | null;
  finanzasDeclaracion: string | null;
  relacionesIdentidad: string | null;
  relacionesDeclaracion: string | null;
  saludIdentidad: string | null;
  saludDeclaracion: string | null;
  ocioIdentidad: string | null;
  ocioDeclaracion: string | null;
  talentosIdentidad: string | null;
  talentosDeclaracion: string | null;
  pazMentalIdentidad: string | null;
  pazMentalDeclaracion: string | null;
  transformacionIdentidad: string | null;
  transformacionDeclaracion: string | null;
  servicioIdentidad: string | null;
  servicioDeclaracion: string | null;
  Meta: Meta[];
}
...
const AREAS_CONFIG = [
  ...
  { key: 'transformacion', label: 'Servicio Transformacional', icon: TrendingUp, color: 'orange', conditional: 'forceTransformationArea' },
  { key: 'servicio', label: 'Servicio Comunitario', icon: Users, color: 'cyan', conditional: 'forceCommunityServiceArea' }
];
...
export default function CoordinadorVerCartaPage() {
  ...
  const fetchCarta = async () => {
    try {
      const res = await fetch(`/api/coordinador/participante/${participanteId}/carta`);
  ...
  const shouldShowArea = (area: typeof AREAS_CONFIG[0]) => {
    if (!area.conditional) return true;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700 rounded-2xl p-6">
            ...
            {carta.mentoringStartDate && (
              <div className="mt-4 pt-4 border-t border-slate-700 flex items-center gap-2 text-sm text-slate-400">
                <Calendar size={16} />
                <span>Fecha de inicio: {new Date(carta.mentoringStartDate).toLocaleDateString('es-ES', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                })}</span>
        ...
        <div className="space-y-4">
          {AREAS_CONFIG.filter(shouldShowArea).map((area) => {
