...
import { Users, Plus, Eye, UserCheck, XCircle, CheckCircle, Shield, AlertTriangle, Target, Key, Lock } from 'lucide-react';
...
export default function LideresPage() {
  ...
  const cambiarPasswordLider = async () => {
    if (!selectedLider || !newPassword) {
    ...
    try {
      const res = await fetch(`/api/director/coordinadores/${selectedLider.id}/password`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        showNotification('success', `Contraseña actualizada para ${selectedLider.nombre}`);
            ...
            <div className="bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 border-2 border-amber-500/30 rounded-3xl p-6 lg:p-8 max-w-md w-full shadow-2xl shadow-amber-500/20 animate-in slide-in-from-bottom duration-300">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">{selectedLider.nombre}</p>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-amber-200/70 text-xs mt-1">
                      El mentor deberá cambiar esta contraseña en su próximo inicio de sesión.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5 mb-6">
                ...
                <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
                  <p className="text-xs text-slate-500 mb-2">Mentor seleccionado:</p>
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-slate-800 rounded-lg">
                      <Shield size={18} className="text-slate-400" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-medium truncate">{selectedLider.nombre}</p>
                      <p className="text-slate-400 text-xs truncate">{selectedLider.email}</p>
                    </div>
                  </div>
                </div>
              </div>
