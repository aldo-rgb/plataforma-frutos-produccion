  ...
  const loadBitacora = async () => {
    try {
      ...
      if (response.ok) {
        ...
        router.push('/dashboard/trainer/bitacoras');
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Cargando Cuestionario...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/dashboard/trainer/bitacoras" className="text-purple-500 hover:underline mt-2 inline-block">
            Volver a la lista
          </Link>
        </div>
      </div>
          ...
          {activeDimension === 2 && (
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-700">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 rounded-lg text-sm text-gray-300 hover:bg-slate-600 transition-colors"
              >
                {showSensitiveData ? (
                  <>
      ...
      return (
        <div className="space-y-6">
              ...
              {d.calificacionPareja && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-blue-400">{d.calificacionPareja}/10</span>
                  </div>
                </div>
              )}
            </>
          ...
          {d.tieneHijos && d.datosHijos?.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-gray-400 mb-3">Hijos</h4>
              <div className="space-y-3">
                {d.datosHijos.map((hijo: any, i: number) => (
                  <div key={i} className="p-3 bg-slate-700/50 rounded-lg">
                    <div className="flex gap-4 text-sm mb-2">
                      <span className="font-medium text-white">{hijo.name}</span>
                      <span className="text-gray-400">{hijo.age} años</span>
                    </div>
                    {hijo.relationship && (
                      <p className="text-sm text-gray-300">{hijo.relationship}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
          ...
          {d.intentoSuicidio && (
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl">
              ...
              {showSensitive ? (
                <p className="text-sm text-gray-300">{d.razonSuicidio || 'No se proporcionaron detalles'}</p>
              ) : (
      ...
      return (
        <div className="py-8">
          <div className="text-center mb-6">
            ...
            <h3 className="text-lg font-semibold text-white">Su Propósito Declarado</h3>
          </div>
...
function ResponseSection({ title, value, highlight = false }: { title: string; value: any; highlight?: boolean }) {
  ...
  return (
    <div className={highlight ? 'p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl' : ''}>
