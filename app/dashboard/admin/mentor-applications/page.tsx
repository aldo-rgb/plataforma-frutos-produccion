...
export default function MentorApplicationsPage() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/mentor-applications?status=${filter}`);
  ...
  const confirmApprove = async () => {
    ...
    setProcessing(true);
    try {
      const response = await fetch(`/api/admin/mentor-applications/${appToApprove}/approve`, {
        method: 'POST'
      });
      ...
      if (data.success) {
        showToast(`Aplicación aprobada exitosamente. Membresía activa hasta: ${new Date(data.membershipExpiryDate).toLocaleDateString('es-MX')}`, 'success');
        fetchApplications();
        setSelectedApp(null);
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    try {
      const response = await fetch(`/api/admin/mentor-applications/${selectedApp.id}/reject`, {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Solicitudes de Mentor</h1>
          <p className="text-slate-400">Revisa y aprueba aplicaciones para certificación de mentores</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-slate-900 rounded-lg border border-slate-800">
            ...
            <p className="text-slate-400">No hay aplicaciones en este estado</p>
          </div>
        ) : (
              ...
              >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        <span className="px-3 py-1 bg-green-500/20 text-green-300 rounded-full text-sm">
                          ${((app.amountPaid || 99900) / 100).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MXN pagado
                        </span>
                      </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-800">
                  <details className="cursor-pointer">
                    <summary className="text-purple-400 font-medium hover:text-purple-300">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-green-500/30 rounded-xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-300 text-center mb-6 leading-relaxed">
              ¿Estás seguro de aprobar esta aplicación?
            </p>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 mb-6 border border-slate-700/50">
              <ul className="space-y-2 text-sm text-slate-300">
                <li className="flex items-start gap-2">
                  ...
                  <span>Se creará el perfil de mentor completo</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" />
                  <span>Membresía activa por <strong className="text-white">1 año</strong></span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" />
                  <span>El usuario obtendrá acceso al panel de mentor</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" />
                  <span>Se enviará notificación de aprobación</span>
                </li>
              </ul>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-6 max-w-md w-full">
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:border-purple-500 focus:outline-none mb-4"
              rows={5}
            />
