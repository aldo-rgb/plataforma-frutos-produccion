    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="mb-6 md:mb-8">
        <div className="text-center md:text-left mb-4">
          ...
          <p className="text-slate-400 text-sm md:text-base mt-1">Administra roles y restablece credenciales de seguridad.</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-6 rounded-lg transition-all flex items-center justify-center gap-2 w-full md:w-auto"
          >
            <UserPlus size={20} />
            Nuevo Usuario
          </Link>
      ...
      {/* LISTA DE USUARIOS - Vista móvil (cards) y desktop (tabla) */}
      <div className="space-y-3">
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2 flex-wrap">
                        <button 
                          ...
                          className="inline-flex items-center gap-2 bg-blue-900/20 hover:bg-blue-900/40 text-blue-400 px-3 py-2 rounded-lg transition-colors border border-blue-500/30"
                          title="Editar"
                        >
                          <Edit size={16} />
                        ...
                        {user.rol === 'PARTICIPANTE' && (
                          <button 
                            ...
                            className="inline-flex items-center gap-2 bg-purple-900/20 hover:bg-purple-900/40 text-purple-400 px-3 py-2 rounded-lg transition-colors border border-purple-500/30"
                        ...
                        <button 
                          ...
                          className="inline-flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-2 rounded-lg transition-colors border border-slate-700"
                        ...
                        <button 
                          ...
                          className="inline-flex items-center gap-2 bg-slate-800 hover:bg-red-900/50 text-slate-400 hover:text-red-400 px-3 py-2 rounded-lg transition-all border border-slate-700"
                          title="Eliminar"
                        >
                          <Trash2 size={16} />
        ...
        <div className="md:hidden space-y-3">
          {usuariosFiltrados.map((user) => (
            <div key={user.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-lg">
              {/* Header de la card */}
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <h3 className="font-bold text-white text-base mb-1">{user.nombre}</h3>
                  <p className="text-xs text-slate-500 break-all">{user.email}</p>
                </div>
                <span className={`flex items-center gap-1.5 text-xs font-bold ml-2 ${user.isActive ? 'text-emerald-400' : 'text-slate-500'}`}>
                  <div className={`w-2 h-2 rounded-full ${user.isActive ? 'bg-emerald-500' : 'bg-slate-500'}`}></div>
                </span>
              </div>
              ...
              {/* Rol Badge */}
              <div className="mb-3">
                <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold border ${getRolColor(user.rol)}`}>
                  {getRolLabel(user.rol)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button 
                  onClick={() => abrirModalEditar(user)}
                  className="flex items-center justify-center gap-2 bg-blue-900/20 text-blue-400 px-3 py-2.5 rounded-lg transition-colors border border-blue-500/30 hover:bg-blue-900/40"
                >
                  <Edit size={16} />
                  <span className="text-xs font-bold">Editar</span>
                </button>
                ...
                <button 
                  onClick={() => abrirModalReset(user)}
                  className="flex items-center justify-center gap-2 bg-slate-800 text-slate-300 px-3 py-2.5 rounded-lg transition-colors border border-slate-700 hover:bg-slate-700"
                >
                  <Key size={16} />
                  <span className="text-xs font-bold">Password</span>
                </button>
                ...
                {user.rol === 'PARTICIPANTE' && (
                  <button 
                    onClick={() => abrirModalCambioMentor(user)}
                    className="flex items-center justify-center gap-2 bg-purple-900/20 text-purple-400 px-3 py-2.5 rounded-lg transition-colors border border-purple-500/30 hover:bg-purple-900/40"
                  >
                    <User size={16} />
                    <span className="text-xs font-bold">Mentor</span>
                  </button>
                ...
                <button 
                  onClick={() => handleDelete(user.id, user.nombre)}
                  className="flex items-center justify-center gap-2 bg-slate-800 text-slate-400 hover:text-red-400 px-3 py-2.5 rounded-lg transition-all border border-slate-700 hover:border-red-500/50 hover:bg-red-900/50"
                >
                  <Trash2 size={16} />
                  <span className="text-xs font-bold">Eliminar</span>
                </button>
