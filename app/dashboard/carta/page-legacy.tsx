...
export default function CartaFrutosPage() {
        ...
        if (data.id) {
          ...
          CATEGORIAS.forEach(cat => {
            ...
            datosReales[cat.id] = {
              avance: avanceValor, 
              tareas: tareasArray,
              declaracion: "" // Por ahora vacío, se llenará cuando agregues el campo en BD
            };
          });
  ...
  // 🔥 NUEVA FUNCIÓN: Guardar cambios manuales sin validación estricta
  const guardarCambiosManuales = async () => {
    setSaving(true);
    
    try {
      // Construir el payload con todas las metas actuales
      const payload: any = {};
      
      CATEGORIAS.forEach(cat => {
        const tareaPrincipal = datos[cat.id]?.tareas.find((t: any) => t.id === 1);
        const tareasAdicionales = datos[cat.id]?.tareas.filter((t: any) => t.id > 1) || [];
        ...
        if (tareaPrincipal) {
          ...
          payload[cat.id] = {
            ...
            declaracion: datos[cat.id]?.declaracion || "",
            avance: datos[cat.id]?.avance || 0,
            recurrence: parsed.recurrenceRule,
            scheduledDays: tareaPrincipal.scheduledDays || [],
            tareasAdicionales: tareasAdicionales.map((t: any) => ({
              descripcion: t.texto,
              completada: t.completado,
              requiereFoto: t.requiereEvidencia
            }))
          };
        } else {
          payload[cat.id] = {
      ...
      setToastMessage({
        ...
        description: 'Tus metas han sido actualizadas exitosamente.'
      });
      setShowToast(true);
      setTimeout(() => setShowToast(false), 3000);
      setIsEditing(false);
      ...
      console.error('Error al guardar cambios manuales:', error);
      setToastMessage({
      ...
      <div className="relative overflow-hidden rounded-3xl bg-slate-900 border border-white/10 p-6 sm:p-10 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col xl:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
            ...
            {/* BOTÓN GUARDAR */}
            <button 
                onClick={isEditing ? guardarCambiosManuales : handleSave}
                disabled={saving}
          ...
          return (
            <div key={cat.id} className="flex flex-col h-full rounded-3xl bg-slate-900 border-2 border-slate-800">
              ...
              <div className="flex-1 p-4 space-y-4">
                ...
                {/* BLOQUE 1: DECLARACIÓN DE PODER (Identidad - "Soy...") */}
                {cat.id !== 'ENROLAMIENTO' && datos[cat.id]?.declaracion && (
                  <div className="p-3 bg-purple-900/20 rounded-lg border-l-4 border-purple-500 shadow-sm">
                    ...
                    <p className="text-sm text-purple-100 italic leading-relaxed">
                      "{datos[cat.id].declaracion}"
                    </p>
                  </div>
                )}
                
                {/* BLOQUE 2: META PRINCIPAL (El Qué) - Solo tarea ID 1 */}
                {(() => {
                  const tareaPrincipal = tareas.find((t: any) => t.id === 1);
                  if (!tareaPrincipal) return null;
                  
                  const isEnrolamiento = cat.id === 'ENROLAMIENTO';
                  
                  return (
                    <div>
                      <label className="text-xs text-slate-500 uppercase font-semibold mb-2 block tracking-wide">
                      ...
                      {isEditing && !isEnrolamiento ? (
                        <div className="space-y-2">
                          <textarea 
                            value={tareaPrincipal.texto || ''}
                            onChange={(e) => handleEditTaskText(cat.id, tareaPrincipal.id, e.target.value)} 
                            className="w-full bg-slate-800/50 text-white text-sm font-medium outline-none resize-none h-16 placeholder-slate-500 rounded-lg p-3 border border-slate-700 focus:border-cyan-500 transition-colors"
                            placeholder="Escribe tu meta medible aquí..."
                          />
                          
                          {/* Botón para programar días */}
                          {(() => {
                            const parsed = parseGoalForTasks(tareaPrincipal.texto || '');
                            if (!parsed.isQuantifiableAndRecurrent) return null;
                            
                            const hasDays = tareaPrincipal.scheduledDays && tareaPrincipal.scheduledDays.length > 0;
                            
                            return (
                              <button
                                onClick={() => {
                                  setCurrentRecurrenceCatId(cat.id);
                                  setSelectedDays(tareaPrincipal.scheduledDays || []);
                                  setShowRecurrencePrompt(true);
                                  (window as any).__editingTaskId = tareaPrincipal.id;
                                }}
                              ...
                              >
                                ...
                                {hasDays 
                                  ? `Días: ${tareaPrincipal.scheduledDays.map((d: string) => 
                        ...
                        <div className="p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                          <p className="text-white font-medium text-sm leading-relaxed">
                            {tareaPrincipal.texto || (
                              <span className="text-slate-500 italic">Sin meta definida</span>
                            )}
                          </p>
                          
                          {/* Mostrar días programados */}
                          {!isEnrolamiento && tareaPrincipal.scheduledDays && tareaPrincipal.scheduledDays.length > 0 && (
                            <div className="mt-2 pt-2 border-t border-slate-700">
                              ...
                              >
                                ...
                                <span className="font-semibold">
                                  {tareaPrincipal.scheduledDays.map((d: string) => 
                ...
                {/* BLOQUE 3: DECLARACIÓN DE PODER EDITABLE (Solo en modo edición) */}
                {isEditing && cat.id !== 'ENROLAMIENTO' && tareas.find((t: any) => t.id === 1) && (
                  <div>
                    <label className="text-xs text-slate-500 uppercase font-semibold mb-2 block tracking-wide flex items-center gap-1">
                      <Sparkles size={12} className="text-purple-400" />
                      Declaración de Poder
                    </label>
                    <textarea
                      value={datos[cat.id]?.declaracion || ''}
                      onChange={(e) => {
                        setDatos((prev: any) => ({
                          ...prev,
                          [cat.id]: {
                      ...
                      className="w-full bg-purple-900/10 text-purple-100 text-sm italic outline-none resize-none h-16 placeholder-purple-400/50 rounded-lg p-3 border border-purple-500/30 focus:border-purple-500/50 transition-colors"
                      placeholder='"Yo soy abundancia infinita, atraigo prosperidad..."'
                    />
                  </div>
                )}
                
                {/* BLOQUE 4: LISTA DE ACCIONES COMPROMETIDAS (El Cómo) - Tareas ID > 1 */}
                {(() => {
                  const tareasAdicionales = tareas.filter((t: any) => t.id > 1);
                  const isEnrolamiento = cat.id === 'ENROLAMIENTO';
                  if (tareasAdicionales.length === 0 && !isEditing) return null;
                  
                  return (
                    <div>
                      <label className="text-xs text-slate-500 uppercase font-semibold mb-2 block tracking-wide">
                        {isEnrolamiento ? 'Invitados' : 'Acciones Comprometidas'}
                      </label>
                      
                      <div className="space-y-3">
                        {tareasAdicionales.map((tarea: any) => {
                          const parsed = parseGoalForTasks(tarea.texto || '');
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                >
                                  {tarea.enRevision ? 
                                    ...
                                    (tarea.completado ? <CheckSquare size={18} /> : <Square size={18} />)
                                  }
                                </button>
                                ...
                                {isEditing && !isEnrolamiento ? (
                                  <input
                                    ...
                                    className="flex-1 bg-slate-900/50 text-white text-sm outline-none placeholder-slate-500 rounded px-2 py-1 border border-slate-700 focus:border-cyan-500"
                                    placeholder="Escribe una acción específica..."
                                  />
                                ) : (
                                  <span className={`flex-1 text-sm ${
                                    tarea.completado ? 'text-slate-500 line-through' : 'text-slate-300'
                                  }`}>
                              ...
                              {/* SELECTOR DE DÍAS para cada acción (MODO EDICIÓN) */}
                              {isEditing && !isEnrolamiento && parsed.isQuantifiableAndRecurrent && (
                                <div className="mt-2 pl-9">
                                  ...
                                  >
                                    ...
                                    {hasDays 
                                      ? `Días: ${tarea.scheduledDays.map((d: string) => 
                                          d.startsWith('dia-') ? d.replace('dia-', '') : d.charAt(0).toUpperCase()
                                        ).join(', ')}`
                                      : '⚠️ Programar Días'
                                    }
                              ...
                              {/* Mostrar días programados (MODO LECTURA) */}
                              {!isEditing && !isEnrolamiento && hasDays && (
                                <div className="mt-2 pl-9">
                                  <button
                                    onClick={() => {
                                      setIsEditing(true);
                        ...
                        {/* Botón para agregar nueva tarea en modo edición */}
                        {isEditing && cat.id !== 'ENROLAMIENTO' && (
                          <button
                            onClick={() => {
                              const nuevoId = Math.max(...tareas.map((t: any) => t.id), 1) + 1;
                              setDatos((prev: any) => ({
                                ...prev,
                                [cat.id]: {
                                  ...prev[cat.id],
                            ...
                            className="w-full py-2 px-3 bg-slate-800/50 hover:bg-slate-800 text-slate-400 hover:text-white text-sm font-medium rounded-lg border border-dashed border-slate-700 hover:border-slate-600 transition-all flex items-center justify-center gap-2"
                          >
