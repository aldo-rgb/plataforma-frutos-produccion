...
import { Shield, Clock, Calendar, Plus, X, AlertCircle, CheckCircle, Trash2, Edit2, Loader2 } from 'lucide-react';
...
export default function DisponibilidadPage() {
  ...
  const cargarBloqueosDisciplina = async () => {
    try {
      ...
      fecha.setDate(fecha.getDate() - fecha.getDay() + diaSeleccionado);
      const fechaStr = fecha.toISOString().split('T')[0];
      
      const res = await fetch(`/api/mentor/disponibilidad/bloqueados?fecha=${fechaStr}`);
      const data = await res.json();
      
      if (data.success) {
        setBloqueosDisciplina(data.horasBloqueadas || []);
      }
    } catch (error) {
