  ...
  try {
    ...
    for (const vision of visionsWithEscrow) {
      ...
      for (const [mentorId, calls] of callsByMentor.entries()) {
        ...
        if (existingPayout) {
          logger.debug(`⚠️ Payout ya existe para mentor ${mentorId}, semana ${weekNumber}`);
          continue;
        }
    ...
    logger.error('❌ Error generando payouts:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('❌ Error obteniendo payouts:', error);
    return NextResponse.json(
