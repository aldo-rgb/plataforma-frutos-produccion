...
// Personajes predefinidos del sistema
...
// POST - Cargar arquetipos predefinidos del sistema
export async function POST(request: Request) {
  try {
    ...
    if (!user || !['DIRECTOR', 'SCHOOL_ADMIN', 'SUPER_ADMIN'].includes(user.rol)) {
      return NextResponse.json({ error: 'Solo administradores pueden cargar arquetipos del sistema' }, { status: 403 });
    }

    // Verificar cuántos ya existen
    const existingCount = await prisma.archetype.count({
      where: { isSystemDefault: true }
    });

    if (existingCount >= SYSTEM_ARCHETYPES.length) {
      return NextResponse.json({ 
        message: 'Los arquetipos del sistema ya están cargados',
    ...
    return NextResponse.json({
      message: `Arquetipos del sistema cargados exitosamente`,
      created,
      skipped,
    ...
    console.error('Error loading system archetypes:', error);
    return NextResponse.json({ error: 'Error al cargar arquetipos' }, { status: 500 });
  }
}

// GET - Obtener lista de arquetipos del sistema
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      expectedTotal: SYSTEM_ARCHETYPES.length
    });
