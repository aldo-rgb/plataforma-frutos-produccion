  ...
  try {
      ...
      try {
        ...
        await prisma.licenseAssignment.create({
          data: {
            ...
            notes: 'Licencia STANDARD automática - Participante por email - Pendiente'
          }
        });
    ...
    for (const user of [...usersInSameOrg, ...usersWithoutOrg]) {
      ...
      if (!user.referralCode) {
        ...
        logger.debug(`🎫 ReferralCode generado para usuario existente ${user.email}: ${updateData.referralCode}`);
      }
