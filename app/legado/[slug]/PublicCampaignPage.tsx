...
interface Campaign {
  ...
  donations: any[];
  expenses: any[];
  plannedExpenses?: Array<{ concept: string; amount: number }>; // Desglose presupuestal
  _count: {
    donations: number;
    members: number;
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
              ...
              {activeTab === 'transparency' && (
                <div>
                  {/* Sección: ¿A dónde va tu dinero? */}
                  {campaign.plannedExpenses && campaign.plannedExpenses.length > 0 && (
                    <div className="mb-8">
                      ...
                      <p className="text-slate-400 text-sm mb-4">
                        Total transparencia. Esto es exactamente cómo se utilizará cada peso donado:
                      </p>

                      <div className="space-y-3">
                        {campaign.plannedExpenses.map((item, idx) => {
                          const percentage = Math.round((item.amount / campaign.goalAmount) * 100);
                          return (
                      ...
                      <div className="mt-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-sm text-emerald-300">
                            Todos los gastos serán auditados y publicados con evidencia fotográfica una vez realizados.
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 sticky top-6">
              ...
              {/* WhatsApp Share */}
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`🌟 ¡Apoya este proyecto de impacto social!\n\n*${campaign.title}*\n\n${campaign.description?.slice(0, 100) || ''}...\n\n💰 Meta: ${formatCurrency(campaign.goalAmount)}\n✅ Recaudado: ${formatCurrency(campaign.raisedAmount)}\n\n👉 ${typeof window !== 'undefined' ? window.location.href : ''}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full mt-2 py-3 bg-[#25D366] hover:bg-[#22c55e] text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2"
              >
