        ...
        {showRegisterModal && (
          ...
          >
            <motion.div
              ...
              className="bg-slate-900 rounded-3xl border border-slate-700 p-8 w-full max-w-md my-8 max-h-[90vh] overflow-y-auto"
                  ...
                  {!showInvoiceStep && !showPaymentOptions ? (
                    ...
                    <div className="mt-6 space-y-4">
                      ...
                      <div className="space-y-3">
                        ...
                        {/* BOTÓN TEMPORAL - Simular pago exitoso */}
                        <button
                          onClick={async () => {
                            setRegistering(true);
                            try {
                              // Primero crear el registro en la DB
                              const checkoutRes = await fetch(`/api/public/evento/${productId}/checkout`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                  nombre: registerForm.nombre,
                              ...
                              if (checkoutData.success && checkoutData.registrationId) {
                                // Simular pago exitoso
                                const verifyRes = await fetch(`/api/public/evento/${productId}/verify-payment`, {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({
                                    registrationId: checkoutData.registrationId,
                                    simulatePayment: true, // Flag para simular
                                  })
                                });
                                const verifyData = await verifyRes.json();
                                
                                if (verifyData.success) {
                                  window.location.href = `/evento/${productId}/success?registration_id=${checkoutData.registrationId}&simulated=true`;
                                } else {
                                  alert('Error: ' + (verifyData.error || 'Error al verificar pago'));
                                }
                              } else {
                                alert('Error: ' + (checkoutData.error || 'Error al crear checkout'));
                              }
                            } catch (err) {
                              console.error(err);
                          ...
                          disabled={registering}
                          className="w-full p-4 bg-gradient-to-r from-green-600 to-emerald-600 border-2 border-green-400 rounded-xl hover:from-green-500 hover:to-emerald-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <div className="flex items-center justify-center gap-3">
                            <CheckCircle className="w-6 h-6 text-white" />
                            <div className="text-left">
                              <p className="font-bold text-white">
                                🧪 PAGAR (Test - Simular pago exitoso)
                              </p>
                              <p className="text-xs text-green-200">Solo para pruebas - Salta la pasarela</p>
                            </div>
                          </div>
                        </button>
                      </div>
