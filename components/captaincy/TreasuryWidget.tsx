        ...
        <div className="space-y-4">
          {shirtOrders.length === 0 ? (
            <div className="text-center py-12 text-white/50">
              ...
              <p>No hay pedidos de playeras</p>
            </div>
          ) : (
            <div className="space-y-3">
              {shirtOrders.map((order) => {
                const statusInfo = shirtStatusLabels[order.status];
                  ...
                  <div key={order.id} className="bg-white/5 rounded-xl p-4 border border-white/10">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white font-medium">{order.user.nombre}</p>
                          <p className="text-white/50 text-sm">
                    ...
                    {isTreasurer && order.status !== 'CANCELLED' && order.status !== 'DELIVERED' && (
                      <div className="flex gap-2 mt-3 pt-3 border-t border-white/10">
                        {order.status === 'PAID' && (
                          <button
                            onClick={() => updateShirtOrderStatus(order.id, 'IN_PRODUCTION')}
                        ...
                        {order.status === 'IN_PRODUCTION' && (
                          <button
                            onClick={() => updateShirtOrderStatus(order.id, 'READY')}
                            className="flex-1 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-medium rounded-lg"
                          >
                            Marcar como Listo
                          </button>
                        )}
                        {order.status === 'READY' && (
                          <button
                            onClick={() => updateShirtOrderStatus(order.id, 'DELIVERED')}
