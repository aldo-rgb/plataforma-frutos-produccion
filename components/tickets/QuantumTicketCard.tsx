...
interface QuantumTicketCardProps {
  eventName: string;
  userName: string;
  ticketCode: string;
...
export function QuantumTicketCard({ 
  eventName, 
  userName, 
  ticketCode, 
  productImage,
  ...
  level = 'BÁSICO',
  animated = true,
}: QuantumTicketCardProps) {
  const isNonTransferable = isTransferable === false;
  ...
  return (
    <Wrapper
      className="relative w-[300px] h-[560px] mx-auto"
      {...wrapperProps}
    >
      {/* Outer Glow */}
      <div 
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1/2 opacity-15">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id={`grid-${ticketCode}`} x="0" y="0" width="25" height="25" patternUnits="userSpaceOnUse">
                <path d="M25 0L0 0 0 25" fill="none" stroke={primaryColor} strokeWidth="0.3" opacity="0.5"/>
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill={`url(#grid-${ticketCode})`}/>
          </svg>
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full px-5 py-5">
          ...
          >
            <div className="space-y-2">
              ...
              <InfoRow label="CLEARANCE" value={`NIVEL ${level.toUpperCase()}`} color={primaryColor} />
