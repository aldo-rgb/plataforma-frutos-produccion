...
import UpgradePaymentModal from '@/components/payment/UpgradePaymentModal';

interface UpgradeToAdvancedWidgetProps {
  advancedStartDate?: Date | string | null;
  price?: number;
  promoPrice?: number;
  promoDeadline?: Date | string | null;
...
export default function UpgradeToAdvancedWidget({ 
  ...
  const handlePaymentSuccess = (option: string, amount: number) => {
    onPaymentSuccess?.(option, amount);
    // El modal se cierra solo después del éxito
  };
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10">
          ...
          {isTrainingActive && promoPrice ? (
            <p className="text-center text-xs text-emerald-400 mt-2">
              🔥 Precio especial: ${promoPrice.toLocaleString()} MXN
            </p>
          ) : price ? (
      ...
      <UpgradePaymentModal
        ...
        userName={userName}
        pricing={{
          advancedPromoPrice: promoPrice,
          advancedRegularPrice: price,
          comboFullPrice: 14500,
          comboDepositPrice: 9000,
          plPromoPrice: 5500,
          isTrainingActive,
          promoDeadline: promoDeadline ? new Date(promoDeadline) : undefined,
          advancedStartDate: advancedStartDate ? new Date(advancedStartDate) : undefined,
        }}
